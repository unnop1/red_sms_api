...
public interface ViewOrderTypeRepo extends JpaRepository<ViewOrderTypeEntity,Long> {
    ...
    @Query(value = "SELECT  odt.*,  (SELECT COUNT(sms_gw.GID)   FROM sms_gateway sms_gw   WHERE odt.MainID = sms_gw.order_type_ID AND ( sms_gw.CREATED_DATE >= trunc(sysdate)  OR sms_gw.CREATED_DATE IS NULL) ) as TotleMsg,  (SELECT COUNT(sms_gw.GID)   FROM sms_gateway sms_gw   WHERE odt.MainID = sms_gw.order_type_ID   AND Is_Status = ?3 AND ( sms_gw.CREATED_DATE >= trunc(sysdate)  OR sms_gw.CREATED_DATE IS NULL)) as TotleSend  FROM  order_type odt  WHERE  Is_Delete = 0 OFFSET ?1 ROWS FETCH NEXT ?2 ROWS ONLY", nativeQuery = true)
