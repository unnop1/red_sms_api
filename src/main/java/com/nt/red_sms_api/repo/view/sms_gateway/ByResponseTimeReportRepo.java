...
public interface ByResponseTimeReportRepo extends JpaRepository<SmsGatewayEntity,Long> {
    ...
    @Query(value = """
            ...
            SELECT 
                receive_month,
                AVG(response_time) AS avg_response_time_per_month,
                MIN(response_time) AS min_response_time_per_month,
                MAX(response_time) AS max_response_time_per_month,
                PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY response_time) AS med_response_time_per_month
            FROM 
                response_times
            GROUP BY 
                receive_month
